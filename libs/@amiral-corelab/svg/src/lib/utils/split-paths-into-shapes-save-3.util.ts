...
function isPointNearInfiniteLine(point: PointLike, start: PointLike, end: PointLike): boolean {
  const edge = subtract(end, start);
  const pointDirection = subtract(point, start);
  const length = Math.hypot(edge.x, edge.y);

  return length > EPSILON && Math.abs(cross(edge, pointDirection)) / length <= ENTRY_VERTEX_DISTANCE_TOLERANCE;
}

function isArcRunSourceOnIncomingSide(
  arcRunVertex: Vertex,
  edgeCommands: EdgeCommand[],
  primitives: Primitive[],
  arcRunStartIndex: number,
): boolean {
  const firstEdgeCommand = edgeCommands[arcRunStartIndex];
  const previousEdgeCommand = edgeCommands[(arcRunStartIndex - ONE + edgeCommands.length) % edgeCommands.length];
  const firstPrimitive = firstEdgeCommand ? primitives[firstEdgeCommand.primitiveIndex] : undefined;
  const previousPrimitive = previousEdgeCommand ? primitives[previousEdgeCommand.primitiveIndex] : undefined;

  if (!firstEdgeCommand || !previousEdgeCommand || firstPrimitive?.kind !== 'arc' || !previousPrimitive) {
    return false;
  }

  return isPointNearInfiniteLine(
    arcRunVertex,
    getPrimitivePoint(previousPrimitive, previousEdgeCommand.tStart),
    getPrimitivePoint(firstPrimitive, firstEdgeCommand.tStart),
  );
}

function shouldUseArcRunCustomVertex(
  arcRunVertex: Vertex,
  arcRunEdgeCommands: EdgeCommand[],
  primitives: Primitive[],
  allEdgeCommands: EdgeCommand[],
  arcRunStartIndex: number,
): boolean {
  return (
    hasMultipleArcPrimitives(arcRunEdgeCommands, primitives) ||
    isArcRunSourceOnIncomingSide(arcRunVertex, allEdgeCommands, primitives, arcRunStartIndex)
  );
}
...
function getFaceVertices(firstNode: GraphNode, edgeCommands: EdgeCommand[], primitives: Primitive[]): Vertex[] {
  ...
  for (let index = ZERO; index < edgeCommands.length; index += ONE) {
    ...
    if (primitive.kind === 'arc') {
      ...
      if (
        arcRunVertex &&
        shouldUseArcRunCustomVertex(arcRunVertex, arcRunEdgeCommands, primitives, edgeCommands, index)
      ) {
        ...
        if (lastEdgeCommand && lastPrimitive) {
          addModelVertex(vertices, getEdgeCommandEndVertex(lastEdgeCommand, lastPrimitive));
        }
      }
