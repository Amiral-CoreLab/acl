...
const toolConsole = new ToolConsole('Figma Icon');
...
await rm(pathsConstant.icons, { recursive: true, force: true });
...
for (const [id, url] of Object.entries(images.images)) {
  ...
  await figmaFetchImageUtil(url, `${pathsConstant.icons}/${name}.svg`, 'svg');
}
